...
fn compile_table(out: &mut String, table: &Value, parents: &Vec<&Value>) -> Result<()> {
    ...
    if parents.is_empty() {
        writeln!(
            out,
            "{} = {{}}", // .script_api top level tables are globals
            table.name.clone().expect("table must have name")
        )?;
    } else {
...
pub fn compile(input: &str) -> Result<String> {
    let toplevel_items = parse(input)?;
    ...
    for item in &toplevel_items {
        let Some(vtype) = &item.vtype else {
            tracing::warn!("top level item without type found, skipping...");
            continue;
        };

        match vtype {
            Type::Single(t) if t == "table" => compile_table(&mut out, item, &Vec::new())?,
            Type::Single(t) if t == "function" => compile_function(&mut out, item, &Vec::new())?,
            _ => {
                tracing::warn!("toplevel {vtype:?} couldn't be compiled");
                continue;
            }
        }
